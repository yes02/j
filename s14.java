Slip14_1: Write a program to accept a number from the user, if number is zero then throw user defined exception “Number is 0” otherwise check whether no is prime or not (Use static keyword).
...
class demo
{
 ...
 no=sc.nextInt();
 if(no==0)
 throw new Zerono("number can not be zero");
 if(no>0)
 {
 for(i=2;i<=no/2;i++)
 {
...
class demo
{
 ...
 public demo()
 {}
 public demo(int rollno,String name) throws Exception
 {
 this.rollno = rollno;
 this.name = name;
